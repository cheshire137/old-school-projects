# This class contains general-purpose methods for other models.
class ModelBase
  ValueRegex = /^[^"']*$/
  TableNameRegex = /^[^"\-'\s]+$/
  ...
  private
    ...
    def ModelBase.get_schema_lines(schema)
      ...
      # Get non-nil, non-blank lines and trim leading, trailing whitespace
      lines.select { |line| !line.nil? && !line.strip.blank? }.map(&:strip)
    end
end
